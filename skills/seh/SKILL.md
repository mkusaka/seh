---
name: seh
description: Use when a coding agent session (Claude Code, Codex, Devin, Pi, or Oh My Pi) should run a shell script on one of its own lifecycle events — after compaction, at session start or end, on each prompt, before or after tool calls, or when the agent stops — using the seh CLI. Covers wiring CLIs with `seh init`, registering scripts for the current or another session with `seh add`, listing and removing registrations, and troubleshooting hooks that do not fire.
---
...
- stdout goes back to the CLI as the hook's output, so each CLI's own hook rules apply (Claude Code adds it to the conversation for `session-start`, `compact`, and `prompt`). Pi and Oh My Pi add any output as hidden context on the next prompt. Use this to re-inject instructions after compaction.
...
3. Oh My Pi: `OMP_SESSION_ID` (set by the omp-shell-context plugin; without it, pass the id explicitly).
...
- Pi / Oh My Pi: `seh init pi` / `seh init omp` installs the seh checkout as a package (`pi install <repo>` / `omp install npm:omp-shell-context <repo>`); they can also install it from GitHub (`pi install git:github.com/mkusaka/seh`, `omp install npm:omp-shell-context github:mkusaka/seh`). Start a new session to load it.
